...
#include <bvsio/bnet_plugin/bnet_plugin.hpp>
...
using bvsio::public_key_type;
using bvsio::chain_id_type;
using bvsio::block_id_type;
using bvsio::block_timestamp_type;
using std::string;
using bvsio::sha256;
using bvsio::signed_block_ptr;
using bvsio::packed_transaction_ptr;
using std::vector;
...
namespace bvsio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              BVS_THROW(block_validate_exception, "bad block" );
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           BVS_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         BVS_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
   ...
   void session::on( const packed_transaction_ptr& p ) {
      ...
      if (!p) {
        ...
        BVS_THROW(transaction_exception, "bad transaction");
      }
