...
bool is_access_violation(fc::unhandled_exception const & e) {
   try {
    ...
    catch (const bvsio::chain::wasm_execution_error& e) {
       return true;
    } catch (...) {
...
bool is_wasm_execution_error(bvsio::chain::wasm_execution_error const& e) {return true;}
...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( config::system_account_name, contracts::bvsio_bios_wasm() );

   set_code( N(test), contracts::bvsio_bios_wasm() );
   set_abi( N(test), contracts::bvsio_bios_abi().data() );
	set_code( N(test), contracts::payloadless_wasm() );
...
BOOST_FIXTURE_TEST_CASE(action_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "assert_false", {} ),
                          bvsio_assert_message_exception, bvsio_assert_message_is("test_action::assert_false") );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_0", raw_bytes), bvsio::chain::wasm_execution_error,
         [](const bvsio::chain::wasm_execution_error& e) {
   ...
   // test read_action_to_64k
   raw_bytes.resize(3);
	BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_64k", raw_bytes ), bvsio::chain::wasm_execution_error,
         [](const bvsio::chain::wasm_execution_error& e) {
            return expect_assert_message(e, "access violation");
         }
      );
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_current_time", fc::raw::pack(now) ),
                          bvsio_assert_message_exception, bvsio_assert_message_is("tmp == current_time()")     );
...
BOOST_FIXTURE_TEST_CASE(cf_action_tests, TESTER) { try {
      ...
      BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_transaction", "send_cf_action_fail", {} ),
                             bvsio_assert_message_exception,
                             bvsio_assert_message_is("context free actions cannot have authorizations") );
...
BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_inline_fail", {}),
                          bvsio_assert_message_exception,
                          bvsio_assert_message_is("test_action::assert_false")                          );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_recurse", {}), bvsio::chain::transaction_exception,
         [](const bvsio::chain::transaction_exception& e) {
...
#warning re-enable deferred transaction replacement test after bug has been fixed
...

   {
      ...
      auth.accounts.push_back( permission_level_weight{{N(testapi), config::bvsio_code_name}, 1} );
      ...
      // But not anymore. Now it should subjectively fail because testapi@additional permission is not unilaterally satisfied by testapi@bvsio.code.
      dtt_action dtt_act3;
...
BOOST_FIXTURE_TEST_CASE(multi_index_tests, TESTER) { try {
   ...
   auto check_failure = [this]( action_name a, const char* expected_error_msg ) {
      BOOST_CHECK_EXCEPTION(  push_action( N(testapi), a, N(testapi), {} ),
                              bvsio_assert_message_exception,
                              bvsio_assert_message_is( expected_error_msg )
      );
   };
...
BOOST_FIXTURE_TEST_CASE(permission_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION( *this, "test_permission", "check_authorization",
      fc::raw::pack( check_auth {
         ...
         .permission = N(active),
         .pubkeys    = {
            public_key_type(string("BVS7GfRtyDWWgxV88a5TRaYY59XmHptyfjsFmHHfioGNJtPjpSmGX"))
         }
      })
   );
...
/*************************************************************************************
 * bvsio_assert_code_tests test cases
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(bvsio_assert_code_tests, TESTER) { try {
   produce_block();
   create_account( N(testapi) );
   produce_block();
   set_code(N(testapi), contracts::test_api_wasm() );
   ...
   const char* abi_string = R"=====(
{
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_assert_code", fc::raw::pack((uint64_t)42) ),
                          bvsio_assert_code_exception, bvsio_assert_code_is(42)                                        );
