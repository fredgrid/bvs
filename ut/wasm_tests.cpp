/**
 *  @file
 *  @copyright defined in bvs/LICENSE.txt
 */
#include <array>
#include <utility>
...
#include <bvsio/chain/wasm_bvsio_constraints.hpp>
...
BOOST_FIXTURE_TEST_CASE( basic_test, TESTER ) try {
   ...
   transaction_id_type yes_assert_id;
   {
      ...
      BOOST_CHECK_THROW(push_transaction( trx ), bvsio_assert_message_exception);
   }
...
BOOST_FIXTURE_TEST_CASE( f32_f64_overflow_tests, tester ) try {
   ...
   auto check = [&](const char *wast_template, const char *op, const char *param) -> bool {
      ...
      try {
         ...
         const auto& receipt = get_transaction_receipt(trx.id());
         return true;
      } catch (bvsio::chain::wasm_execution_error &) {
         return false;
      }
   };
   //
...
BOOST_FIXTURE_TEST_CASE( big_memory, TESTER ) try {
   ...
   string biggest_memory_wast_f = fc::format_string(biggest_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES", bvsio::chain::wasm_constraints::maximum_linear_memory/(64*1024)));
   ...
   string too_big_memory_wast_f = fc::format_string(too_big_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES_PLUS_ONE", bvsio::chain::wasm_constraints::maximum_linear_memory/(64*1024)+1));
   BOOST_CHECK_THROW(set_code(N(bigmem), too_big_memory_wast_f.c_str()), bvsio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( table_init_tests, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(tableinit), too_big_table), bvsio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( memory_init_border, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_toolong), bvsio::chain::wasm_execution_error);
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_negative), bvsio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( nested_limit_test, TESTER ) try {
   ...
   }
   {
      ...
      for(unsigned int i = 0; i < 1024; ++i)
         ss << ")";
      ss << "))";
      BOOST_CHECK_THROW(set_code(N(nested), ss.str().c_str()), bvsio::chain::wasm_execution_error);
   }
...
BOOST_FIXTURE_TEST_CASE( lotso_globals, TESTER ) try {
   ...
   .c_str()), bvsio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( offset_check, TESTER ) try {
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << bvsio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << bvsio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0)))";
   ...
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << bvsio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << bvsio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << bvsio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << bvsio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0)))";
      ss << ") (export \"apply\" (func $apply)) )";

      BOOST_CHECK_THROW(set_code(N(offsets), ss.str().c_str()), bvsio::chain::wasm_execution_error);
      produce_block();
   }
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << bvsio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << bvsio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
...
// abi_serializer::to_variant failed because bvsio_system_abi modified via set_abi.
// This test also verifies that chain_initializer::bvs_contract_abi() does not conflict
// with bvsio_system_abi as they are not allowed to contain duplicates.
BOOST_FIXTURE_TEST_CASE(bvsio_abi, TESTER) try {
   produce_blocks(2);
   ...
   // verify to_variant works on bvs native contract type: newaccount
...
BOOST_FIXTURE_TEST_CASE( check_table_maximum, TESTER ) try {
   ...
   BOOST_CHECK_THROW(push_transaction(trx), bvsio::chain::wasm_execution_error);
   }
   ...
   signed_transaction trx;
   action act;
   act.name = bvsio::chain::wasm_constraints::maximum_table_elements+54334;
   act.account = N(tbl);
   act.authorization = vector<permission_level>{{N(tbl),config::active_name}};
   trx.actions.push_back(act);
      set_transaction_headers(trx);
   ...
   BOOST_CHECK_EXCEPTION(push_transaction(trx), bvsio::chain::wasm_execution_error, [](const bvsio::chain::wasm_execution_error &e) {return true;});
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE( big_maligned_host_ptr, TESTER ) try {
   ...
   string large_maligned_host_ptr_wast_f = fc::format_string(large_maligned_host_ptr, fc::mutable_variant_object()
                                              ("MAX_WASM_PAGES", bvsio::chain::wasm_constraints::maximum_linear_memory/(64*1024))
                                              ("MAX_NAME_ARRAY", (bvsio::chain::wasm_constraints::maximum_linear_memory-1)/sizeof(chain::account_name)));
...
BOOST_FIXTURE_TEST_CASE(net_usage_tests, tester ) try {
   ...
   (import "env" "bvsio_assert" (func $bvsio_assert (param i32 i32)))
      (table 0 anyfunc)
      (memory $0 1)
      ...
      produce_blocks(1);
      signed_transaction trx;
      auto wasm = ::bvsio::chain::wast_to_wasm(code);
