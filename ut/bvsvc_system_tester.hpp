...
namespace bvsio_system {

class bvsio_system_tester : public TESTER {
public:

   bvsio_system_tester()
   : bvsio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   bvsio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(bvsio.token), N(bvsio.ram), N(bvsio.ramfee), N(bvsio.stake),
               N(bvsio.bpay), N(bvsio.vpay), N(bvsio.saving), N(bvsio.names) });

      produce_blocks( 100 );

      set_code( N(bvsio.token), contracts::bvsio_token_wasm() );
      set_abi( N(bvsio.token), contracts::bvsio_token_abi().data() );

      {
         const auto& accnt = control->db().get<account_object,by_name>( N(bvsio.token) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer_max_time);
      }

      create_currency( N(bvsio.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "bvsio" ) );

      set_code( config::system_account_name, contracts::bvsio_system_wasm() );
      set_abi( config::system_account_name, contracts::bvsio_system_abi().data() );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("bvsio")  + get_balance("bvsio.ramfee") + get_balance("bvsio.stake") + get_balance("bvsio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& bvsin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",bvsin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(bvsio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(bvsio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(bvsio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = bvsio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(bvsio.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "bvsio_global_state", data, abi_serializer_max_time );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(bvsio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "bvsio", "bvsio.msig", core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(bvsio.msig), contracts::bvsio_msig_wasm() );
         set_abi( N(bvsio.msig), contracts::bvsio_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(bvsio.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total BVS supply to activate chain
      transfer( "bvsio", "alice1111111", core_from_string("650000000.0000"), "bvsio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::bvsio_code_name}, 1},
...
inline uint64_t M( const string& bvs_str ) {
   return core_from_string( bvs_str ).get_amount();
}
