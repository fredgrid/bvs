 ...
 */
#include "proxy.hpp"
#include <bvsio/transaction.hpp>

using namespace bvsio;

proxy::proxy( bvsio::name self, bvsio::name first_receiver, bvsio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      bvsio::token::transfer_action a( "bvsio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, bvsio::ignore<std::vector<char>> ) {
