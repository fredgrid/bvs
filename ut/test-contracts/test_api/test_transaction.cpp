...
struct test_action_action {
   static bvsio::name get_account() {
      return bvsio::name{ACCOUNT};
   }

   static bvsio::name get_name() {
      return bvsio::name{NAME};
   }
...
void test_transaction::send_action_large() {
   ...
   act.send();
   bvsio_assert( false, "send_message_large() should've thrown an error" );
}
...
void test_transaction::test_tapos_block_prefix() {
   ...
   bvsio_assert( tbp == tapos_block_prefix(), "tapos_block_prefix does not match" );
}
...
void test_transaction::test_tapos_block_num() {
   ...
   bvsio_assert( tbn == tapos_block_num(), "tapos_block_num does not match" );
}
...
void test_transaction::test_read_transaction() {
   ...
   bvsio_assert( size == read, "read_transaction failed");
   h = bvsio::sha256(buf, read);
   print(h);
}
...
void test_transaction::test_transaction_size() {
   ...
   bvsio_assert( trans_size == transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_transaction_empty( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   bvsio_assert( false, "send_transaction_empty() should've thrown an error" );
}
...
void test_transaction::assert_false_error_handler( const bvsio::transaction& dtrx ) {
   bvsio_assert( dtrx.actions.size() == 1, "transaction should only have one action" );
   bvsio_assert( dtrx.actions[0].account == "testapi"_n, "transaction has wrong code" );
   bvsio_assert( dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name" );
   bvsio_assert( dtrx.actions[0].authorization.size() == 1, "action should only have one authorization" );
   bvsio_assert( dtrx.actions[0].authorization[0].actor == "testapi"_n, "action's authorization has wrong actor" );
   bvsio_assert( dtrx.actions[0].authorization[0].permission == "active"_n, "action's authorization has wrong permission" );
}
...
void test_transaction::send_transaction_large( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   bvsio_assert( false, "send_transaction_large() should've thrown an error" );
}
...
void test_transaction::cancel_deferred_transaction_success() {
   ...
   bvsio_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace bvsio;
   auto r = cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   bvsio_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   bvsio_assert( false, "send_cfa_action_fail() should've thrown an error" );
}

void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( bvsio::name{"testtrans"}.value, bvsio::name{"table"}.value, bvsio::name{"testtrans"}.value, 0, buf, 4 );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   trx.send( sender_id, bvsio::name{receiver} );
}
