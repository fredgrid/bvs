...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(bvsio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(bvsio.token), contracts::bvsio_token_wasm() );
   c.set_abi( N(bvsio.token), contracts::bvsio_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( N(bvsio.token), N(create), N(bvsio.token), mutable_variant_object()
              ("issuer",       "bvsio" )
   ...
   cr = c.push_action( N(bvsio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
