...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(bvsio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create bvsio.msig and bvsio.token
        create_accounts({N(bvsio.msig), N(bvsio.token), N(bvsio.ram), N(bvsio.ramfee), N(bvsio.stake), N(bvsio.vpay), N(bvsio.bpay), N(bvsio.saving) });
        // Set code for the following accounts:
        //  - bvsio (code: bvsio.bios) (already set by tester constructor)
        //  - bvsio.msig (code: bvsio.msig)
        //  - bvsio.token (code: bvsio.token)
        // set_code_abi(N(bvsio.msig), contracts::bvsio_msig_wasm(), contracts::bvsio_msig_abi().data());//, &bvsio_active_pk);
        // set_code_abi(N(bvsio.token), contracts::bvsio_token_wasm(), contracts::bvsio_token_abi().data()); //, &bvsio_active_pk);

        set_code_abi(N(bvsio.msig),
                     contracts::bvsio_msig_wasm(),
                     contracts::bvsio_msig_abi().data());//, &bvsio_active_pk);
        set_code_abi(N(bvsio.token),
                     contracts::bvsio_token_wasm(),
                     contracts::bvsio_token_abi().data()); //, &bvsio_active_pk);

        // Set privileged for bvsio.msig and bvsio.token
        set_privileged(N(bvsio.msig));
        set_privileged(N(bvsio.token));

        // Verify bvsio.msig and bvsio.token is privileged
        const auto& bvsio_msig_acc = get<account_object, by_name>(N(bvsio.msig));
        BOOST_TEST(bvsio_msig_acc.privileged == true);
        const auto& bvsio_token_acc = get<account_object, by_name>(N(bvsio.token));
        BOOST_TEST(bvsio_token_acc.privileged == true);


        // Create SYS tokens in bvsio.token, set its manager as bvsio
        ...
        create_currency(N(bvsio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to bvsio.system
        issue(N(bvsio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(bvsio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "bvsio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), bvsio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), bvsio_assert_message_exception);
