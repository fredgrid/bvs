...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for bvsio reqauth action with alice's spending key
   chain.link_authority("alice", "bvsio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "bvsio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with bvsio reqauth
   chain.unlink_authority("alice", "bvsio", "reqauth");
   ...
   // Link authority for any bvsio action with alice's scud key
   chain.link_authority("alice", "bvsio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "bvsio", "first",  "reqauth");
...
try {
   ...
   // Creating account with bvsio. prefix with privileged account
   chain.create_account("bvsio.test1");

   // Creating account with bvsio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("bvsio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'bvsio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "bvsio", "bvsio.any", "reqauth" );
   chain.link_authority( "bob", "bvsio", "bvsio.any", "reqauth" );

   /// this should succeed because bvsio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = bvsio::chain::resource_limits::resource_usage_object;
   using by_owner = bvsio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link bvsio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
