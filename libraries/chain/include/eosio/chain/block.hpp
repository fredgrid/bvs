#pragma once
#include <bvsio/chain/block_header.hpp>
...
FC_REFLECT_ENUM( bvsio::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(bvsio::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(bvsio::chain::transaction_receipt, (bvsio::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(bvsio::chain::signed_block, (bvsio::chain::signed_block_header), (transactions)(block_extensions) )
