...
#define N(X) bvsio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<bvsio::chain::name> : private hash<uint64_t> {
      typedef bvsio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const bvsio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, bvsio::chain::name& check);
} // fc


FC_REFLECT( bvsio::chain::name, (value) )
