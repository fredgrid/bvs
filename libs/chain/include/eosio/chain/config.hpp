...
namespace bvsio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(bvsio);
const static uint64_t null_account_name      = N(bvsio.null);
const static uint64_t producers_account_name = N(bvsio.prods);
...
const static uint64_t bvsio_auth_scope       = N(bvsio.auth);
const static uint64_t bvsio_all_scope        = N(bvsio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t bvsio_any_name = N(bvsio.any);
const static uint64_t bvsio_code_name = N(bvsio.code);
...
const static bvsio::chain::wasm_interface::vm_type default_wasm_runtime = bvsio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace bvsio::chain::config

constexpr uint64_t BVS_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / bvsio::chain::config::percent_100;
}

template<typename Number>
Number BVS_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + bvsio::chain::config::percent_100 - bvsio::chain::config::percent_1)  / bvsio::chain::config::percent_100;
}
