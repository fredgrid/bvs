/**
 *  @file
 *  @copyright defined in bvsio/LICENSE.txt
 */
...
{
   try {
      app().set_version(bvsio::nodbvs::config::version);

      auto root = fc::app_path();
      app().set_default_data_dir(root / "bvsio/nodbvs/data" );
      app().set_default_config_dir(root / "bvsio/nodbvs/config" );
      ...
      initialize_logging();
      ilog("nodbvs version ${ver}", ("ver", app().version_string()));
      ilog("bvsio root is ${root}", ("root", root.string()));
      ilog("nodbvs using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("nodbvs data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
   ...
   ilog("nodbvs successfully exiting");
   return SUCCESS;
}
